...
def ingest_batch(
    ...
    embeddings = encoder.embed_images(images)  # (B, D)
    ...
    for i, (emb, fp) in enumerate(zip(embeddings, valid_paths)):
        ...
        if cam.occupied.any():
            ...
            sims = (q_norm @ cam._keys_norm[valid_idx].float().T).squeeze(0)
        ...
        if webhook_url:
            try:
                payload = {
                    ...
                    "timestamp": now,
                    "neighbors": [
                        {"path": n[0], "similarity": n[1]}
                        for n in neighbors if n[0] is not None
                    ]
                }
                requests.post(webhook_url, json=payload, timeout=5)
            except Exception as e:
                logger.error("Failed to send webhook for %s: %s", fp.name, e)

        ingested += 1

    t_batch1 = time.perf_counter()
    if ingested > 0:
        load_s = t_after_load - t_batch0
        embed_s = t_after_embed - t_embed0
        learn_s = t_batch1 - t_learn0
        total_s = t_batch1 - t_batch0
        logger.info(
            "Batch timings: files=%d load=%.3fs embed=%.3fs learn+sqlite=%.3fs total=%.3fs "
            "(per-image: embed=%.3fs total=%.3fs)",
            ingested,
            load_s,
            embed_s,
            learn_s,
...
def watch_and_ingest(
    ...
    while True:
        ...
        if should_flush and pending:
            ...
            try:
                n = ingest_batch(cam, encoder, batch, metadata_conn, webhook_url=webhook_url)
                total_ingested += n
