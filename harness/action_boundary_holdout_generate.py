#!/usr/bin/env python3
"""PR-12.7 Stage A — standalone holdout generation driver (memo §23, erratum E1).

Registered in PR12_7_ACTION_BOUNDARY_HOLDOUT.md §23.3: the witness-window
(W1/W2) emission path in the byte-frozen ``harness_boundary_sim.py`` is
dispatched by hardcoded scan-block keys, so C/E holdout packets cannot be
emitted config-only (§23.1). This driver supplies the registered,
byte-verified *invocation* of the frozen primitives instead: it IMPORTS
``run_cell`` from the frozen emitter (never edits it; the emitter sha256
is pinned and checked before import and after every run) and calls it with
the identical keyword signature the registered PR-12.4 W2 runner uses
(``run_scan12_4``, harness_boundary_sim.py:1897-1901).

Mandatory pre-emission byte-equivalence self-check (§23.4): before any
holdout packet may be emitted, this driver must regenerate the committed
PR-12.4 W2 parity anchor
``results/issue_failure_mode_blindness/pr12_4/W2/pairD_oneshot_s1/``
into a temp directory and prove byte-identical ``memory_packet.jsonl``,
``audit_packet.jsonl``, and ``decision_table.csv`` (full byte compare +
sha256 + explicit schema/field-set + record-ordering assertions), recorded
in a provenance JSON. Any mismatch is kill §23.5.1 and generation aborts.

Stage separation (§23.7): running the self-check proves invocation
fidelity only. Emitting C/E holdout packets (``--emit-holdout``) requires
its own separate explicit authorization AND a ``scan12_7_holdout`` block
in ``harness_policy.json`` whose cells byte-match the §4 registered table;
absent either, emission refuses. Stage B scoring is a different program
(``action_boundary_holdout_score.py``, not this file, still unauthorized).
This driver never reads truth labels, registry labels, or any pr12_5/
pr12_6 scoring output; it scans its own output for scoring signals and
aborts on any hit (§23.6). No FAM-core file is imported. PR-10
...
HOLDOUT_SCAN_KEY = "scan12_7_holdout"
HOLDOUT_CACHE = "results/issue_failure_mode_blindness/pr12_7_holdout_cache"
...
REGISTERED_HOLDOUT_CELLS = {
    "pairC_oneshot_s1": (f"{GOVERNED}/per_probe_stale-oneshot_pairC_s1",
                         f"{GEOMETRY}pairC/oneshot/s1"),
    "pairC_oneshot_s2": (f"{GOVERNED}/per_probe_stale-oneshot_pairC_s2",
                         f"{GEOMETRY}pairC/oneshot/s2"),
    "pairE_oneshot_s1": (f"{GOVERNED}/per_probe_stale-oneshot_pairE_s1",
                         f"{GEOMETRY}pairE/oneshot/s1"),
    "pairE_oneshot_s2": (f"{GOVERNED}/per_probe_stale-oneshot_pairE_s2",
                         f"{GEOMETRY}pairE/oneshot/s2"),
    "pairC_contra_s1": (f"{GOVERNED}/per_probe_contra_pairC_s1",
                        f"{GEOMETRY}pairC/contra/s1"),
    "pairC_contra_s2": (f"{GOVERNED}/per_probe_contra_pairC_s2",
                        f"{GEOMETRY}pairC/contra/s2"),
    "pairE_contra_s1": (f"{GOVERNED}/per_probe_contra_pairE_s1",
                        f"{GEOMETRY}pairE/contra/s1"),
    "pairE_contra_s2": (f"{GOVERNED}/per_probe_contra_pairE_s2",
                        f"{GEOMETRY}pairE/contra/s2"),
    "clean_pairC_s1": (f"{GOVERNED}/per_probe_clean_pairC_s1",
                       f"{GEOMETRY}pairC/clean/s1"),
    "clean_pairE_s1": (f"{GOVERNED}/per_probe_clean_pairE_s1",
                       f"{GEOMETRY}pairE/clean/s1"),
}
...
def self_check(provenance: dict) -> None:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="pr12_7_anchor_parity_"))
    try:
...
def emit_holdout(provenance: dict) -> None:
    ...
    if block is None:
        die(provenance,
            ...
            "— Stage A emission is separately authorized (§23.7) and its "
    ...
    if got != registered:
        die(provenance, "kill §23.5.3: scan12_7_holdout cells deviate from "
                        "the §4 registered table")
    ...
    run_cell = load_run_cell()
    twin = Path(tempfile.mkdtemp(prefix="pr12_7_holdout_twin_"))
    try:
...
def main() -> None:
    ...
    provenance = {
        "driver": "harness/action_boundary_holdout_generate.py",
        "registration": "PR12_7_ACTION_BOUNDARY_HOLDOUT.md §23 (erratum E1)",
