...
def run_g10_drift_full_stack(root_dir, device="cuda"):
    """G10 Drift Test with full stack: Quantized Storage + NSTP + Adapter.

    Mirrors ``run_g10_drift`` but enables:
    * ``use_bfloat16=True``  — quantized (bfloat16) prototype storage
    * :class:`~adapter.MetricAdapter` (512-d projection) — learned metric space
    * :class:`~nstp.NSTPController` — post-retrieval lateral inhibition

    Success criterion: BWT > -5 % (no catastrophic forgetting).
    """
    ...
    bwt = acc_clean_2 - acc_clean_1
    print(f"\n--- G10 (Full Stack) Results — G20 Continual-Learning Stress ---")
    print(f"  Stack: bfloat16 storage | MetricAdapter(1024→512) | NSTPController")
    print(f"Backward Transfer: {bwt:+.2f}%")

    if bwt > -5.0:
        print("\u2705 PASS: Full-stack FAM supports multi-modal concepts (Clean + Blur coexist).")
        print("  G20 kill condition NOT triggered.")
    else:
        print("\u274c FAIL: Catastrophic Forgetting/Interference with full stack.")
        print("  \u26a0\ufe0f  G20 KILL CONDITION MET (BWT \u2264 -5%)")
