...
* Organic cell — a synthetic re-ingest ledger built from the original
  ...
  new file with scripted timestamps. The memo's registered *cell* name
  stays "organic cell" (§7) — the on-disk directory is named
  ``cells_data/synthetic_reingest/`` for what it actually holds, so no
  reader can mistake scripted events for observed history.
...
# Committed cell data. The organic cell's corpus is scripted, and the
# directory says so; nothing else in the tree may hardcode this path.
CELLS_DATA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "cells_data")
SYNTHETIC_REINGEST_DIR = os.path.join(CELLS_DATA, "synthetic_reingest")
SYNTHETIC_LEDGER = os.path.join(SYNTHETIC_REINGEST_DIR, "ledger.sqlite3")
SYNTHETIC_MANIFEST = os.path.join(SYNTHETIC_REINGEST_DIR, "manifest.json")
SYNTHETIC_QUERIES = os.path.join(SYNTHETIC_REINGEST_DIR, "queries.json")
SYNTHETIC_SESSIONS = os.path.join(SYNTHETIC_REINGEST_DIR, "sessions.json")

# The original defunct-project fixture: read-only source of the path
# list, never written, pinned by sha in the cell definition.
ORIGINAL_FIXTURE_DB = os.path.join("test_data", "metadata.db")
ORIGINAL_FILES_ROOT = "test_drop"
