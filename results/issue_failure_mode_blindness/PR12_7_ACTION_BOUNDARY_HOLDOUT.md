...
## 25. Stage A holdout generation run (append-only; §§1–24 frozen)

*Run record under the §19/§22+ append-only convention. Separate explicit
authorization executed here: (i) merge of the §24 generator to main
(non-ff `204df97`, both forges verified), and (ii) the additive
`scan12_7_holdout` config manifest plus the Stage A C/E emission run —
**nothing else**. Stage B scoring remains unauthorized:
`action_boundary_holdout_score.py` does not exist, no `pr12_7/` scoring
directory was created, `action_boundary_score.py` was not run, and no
W2:F1b decision was computed on any holdout row. No FAM-core file was
imported or modified; no PR-12.1–12.6 artifact/verdict/gate/output
changed; PR-10 merge-abstain remains the only certified reader contract;
posture remains deferral.*

### 25.1 Config manifest

Additive `scan12_7_holdout` block committed to `harness_policy.json`
(+59/−0; every pre-existing key verified byte-equal against HEAD by JSON
comparison): the ten §4-registered cells exactly (pairs C/E ×
oneshot/contra × s1/s2, plus `clean_pairC_s1`/`clean_pairE_s1`
controls), shapes `{prototype, W1, W2}` (the registered parity set),
`policy_version pr12.7-holdout-0.1`. The block is a data manifest only —
read by the standalone generator, reachable by no emitter CLI path.
Pre-flight (before the edit): all 50 run-stem input files present and
git-committed; all 10 `pr4_geometry_table.json#governance` keys present;
the §16 G-H1 exposure grep re-run — zero `pairC|pairE` hits in any
12.1–12.6 scan config or any committed `pr12_1/`–`pr12_6/` aggregate.

### 25.2 Run and proofs

Run 2026-07-08 (UTC), darwin, Python 3.13.5:
`python3 harness/action_boundary_holdout_generate.py --emit-holdout`
(exit 0, verdict `stageA-emitted`). Recorded in
`pr12_7_holdout_cache/stage_a_provenance.json` (sha256 `1868e1d1…`):

* **Anchor gate re-passed first (§23.4):** with the manifest present,
  `W2/pairD_oneshot_s1` again regenerated byte-identical to the
  committed anchor (memory `fc451528…`, audit `f02c9037…`, decision
  `f425c350…`) — the additive block is thereby proven non-perturbing to
  the frozen emission path before any holdout byte was written.
* **Emission:** 10 cells × 3 shapes through the imported frozen
  `run_cell`, keyword-identical to the registered W2 runner; every
  cell-shape **double-emitted** (cache + independent temp twin) and all
  30 proven byte/sha256/schema/order-identical (§23.5.6). Emitter sha256
  = pin `2539686a…` before the run, after the self-check, and after
  emission. Frozen-surface `git status` check empty.
* **Cache purity (§23.6):** generator token scan zero hits on all 30
  cell-shapes; independently re-verified by external grep over the whole
  cache (no `true_label`, `vote_pred`, verdict tokens, or
  `pr12_5//pr12_6/` references).
* **Output:** 91 files — 30 cell-shape dirs × {`memory_packet.jsonl`,
  `audit_packet.jsonl`, `decision_table.csv`} + the provenance JSON;
  all 90 per-file sha256s in `stage_a_provenance.json`
  (`holdout_output_sha256`), with per-cell input hash tables
  (`holdout_input_sha256`, 7 pinned inputs per cell). Packet-tree
  aggregate sha256 (sorted per-file sha256 list, provenance excluded):
  `028720c61073adb6e47260e91d5f478ef2b3c90e059df40e5371442ad0b3cc77`.
  W2 row counts (structural, from `decision_table.csv`; no scoring):
  pairC_oneshot 2757/2654 (s1/s2), pairE_oneshot 2646/2797,
  pairC_contra 2488/2551, pairE_contra 2902/2956, clean_pairC_s1 2244,
  clean_pairE_s1 2724.

### 25.3 Boundary

Stage A is complete: the registered holdout packets now exist under
`pr12_7_holdout_cache/`, generated by the registered primitive under
the §23.4 gate. They carry read-time observables only. Whether they
constitute a *valid, powered* holdout (G-H1–G-H5, including the G-H4
floor) is exactly the question Stage B is registered to answer and is
**not** claimed here. Stage B scoring requires separate explicit
approval. No deployment, live acting, prompting use, promotion, memory
ingestion, autonomous downstream use, or reader-contract change — PR-10
merge-abstain remains the only certified reader contract.

## 26. Stage B holdout scoring run — verdict `holdout-validity-GO(W2:F1b)` (append-only; §§1–25 frozen)

*Results section under the §19/§22+ append-only convention, recorded
after a separately-authorized Stage B run. Every number below is a
verbatim field of `pr12_7/holdout_scan.json`.*

### 26.1 Run record

Scorer: `harness/action_boundary_holdout_score.py` (new standalone
read-only file per §4 Stage B; stdlib + subprocess-git; writes only
under `pr12_7/` and temp). Run 2026-07-08, darwin, repo at the Stage A
merge `480cf1d` (cache pin), policy pin `0afcb2b` unchanged. Command:
`python3 harness/action_boundary_holdout_score.py` (exit 0, twice).
`harness_boundary_sim.py`, `action_boundary_score.py`, and
`reader_utility_score.py` byte-untouched; no `pr12_5/`/`pr12_6/` file
read at all (no reconciliation is registered for 12.7 and none ran).

### 26.2 Holdout-validity gates (§16) — evaluated first, all PASS

* **G-H1 (unexposedness):** exposure re-grep at scoring time over every
  12.1–12.6 scan config block and every committed `pr12_1/`–`pr12_6/`
  filename and `*.json` content: **empty match set** for pairC/pairE.
* **G-H2 (provenance integrity):** all **30/30** cache cell-shapes
  regenerated byte-identically from pinned committed inputs through the
  unmodified emitter at scoring time (sha256 `2539686a…` before and
  after, equal to pin); 50/50 manifest inputs match their pins
  (cache files ↦ `480cf1d`, governed truth CSVs ↦ `0afcb2b`).
* **G-H3 (no-tuning attestation):** the §5 policy block embedded in the
  holdout scorer extracts sha256-identical to the frozen
  `action_boundary_score.py` source (block sha `2f009cf29ce64dc2…`);
  parameter-free F1 policies, zero F2 registered, no fitting code path.
* **G-H4 (power floor):** **897** in-scope W2 one-shot rows aggregated
  (floor 30); max in-scope W2 contra rows per pair: pairC **7**,
  pairE **12** (floor ≥5 in ≥1 unit per pair) — **powered**, with the
  honest caveat that the contra panels are thin (7/0/12/5 rows).
* **G-H5 (feature-reconstructability):** 3,116 rows classified from
  packet fields alone (structural; truth joined post-decision only).

§17 clean: zero kill conditions; committed dirs clean before and after;
internal double pass identical; external second invocation reproduced
all **121** `pr12_7/` files byte-identically.

### 26.3 W2:F1b on the holdout units (gates §§12–15) — sole passing combo

| unit (W2) | arm | n | coverage | precision | wrong-mass rate |
|---|---|---|---|---|---|
| pairC_oneshot_s1 | oneshot | 176 | 1.0 | 1.0 | 0.0 |
| pairC_oneshot_s2 | oneshot | 171 | 1.0 | 1.0 | 0.0 |
| pairE_oneshot_s1 | oneshot | 265 | 0.9434 | 0.90 | 0.0943 |
| pairE_oneshot_s2 | oneshot | 285 | 0.9825 | 1.0 | 0.0 |
| pairC_contra_s1 | contra | 7 | 0.0 | — | 0.0 |
| pairC_contra_s2 | contra | 0 | — | — | — |
| pairE_contra_s1 | contra | 12 | 0.0 | — | 0.0 |
| pairE_contra_s2 | contra | 5 | 0.0 | — | 0.0 |

G-A1 pass ×4 (min precision 0.90 ≥ 0.75); G-A2 aggregate one-shot
coverage **0.9777** ≥ 0.25; G-A3 pass ×4 (the certified-only P-assert
reader scores 0.0 on acted rows of every one-shot unit — the deployed
vote remains anti-signal on the one-shot harm class at held-out pairs);
G-C1/G-C2 pass ×4 (F1b acts on **zero** contra rows; wrong mass 0);
G-C3 all four contra units included; §15 per-unit ceiling pass ×8
(worst 0.0943 < 0.10, pairE_oneshot_s1 — inside the pairD-like band
12.6 recorded); §14 global wrong-mass rate **0.0271** ≤ 0.05 over the
8 gated units. `combo_pass`: **only `W2:F1b`** — W2:F1a and W2:F1c
fail G-C1 on pairE_contra_s1 (coverage 0.167, precision 0.0, wrong
mass 0.167), and every W1 combo fails coverage.

### 26.4 The finding

The PR-12.6 quiet-cell guard **generalizes on the pair axis**: on
packets from pairs the policy family was never frozen against, the
guard (`n_contradiction_pairs ≤ n_ambiguous_pairs`, from the cell's own
audit packet) is open on all four held-out one-shot cells and closed on
all four held-out contra cells — the same perfect harm-class separation
as 12.6, and again the comparators without the guard (F1a/F1c) leak
wrong-action mass on a held-out contra cell while F1b does not. H0
(§1) is rejected on this panel: the 12.6 boundary is not an artifact
of pairs B/D. Scope limits stated plainly: pair-axis holdout at
existing seeds s1/s2 only (no fresh-seed claim, §3); contra safety
shown by total non-action on thin contra panels (7/0/12/5 in-scope
rows); pairE_oneshot_s1 wrong mass 0.0943 sits close under the 0.10
per-unit ceiling.

### 26.5 Boundary (unchanged)

Per §21: this GO establishes **offline holdout evidence that the 12.6
action boundary generalizes to the held-out pairs — and nothing beyond
that**. No deployment, live acting, prompting use, promotion to any
policy version, memory ingestion, autonomous downstream use, LLM/agent
reader certification, or reader-contract change. **PR-10 merge-abstain
remains the only certified reader contract**, and the operational
posture on witness-window rows remains **deferral** unless and until a
separate pre-registration proposes otherwise with this PR's evidence as
one input. PR-12.1–12.6 verdicts stand unchanged.
